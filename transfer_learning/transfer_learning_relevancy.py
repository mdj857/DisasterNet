...
Y_relevancy_old = combined['Q4Relevancy'].tolist()
print set(Y_relevancy_old)
...
feature_vecs = []
Y_relevancy = []
for a, b in zip(feature_vecs_old, Y_relevancy_old):
    if b != u' ':
        feature_vecs.append(a)
        Y_relevancy.append(str(b))
        
print len(Y_relevancy)
print set(Y_relevancy)
...
X=feature_vecs.reshape(2245, 25088)
# converting target variable to array
Y=np.asarray(Y_relevancy)
...
callbacks = [EarlyStopping(monitor='val_loss', patience=2),
             ModelCheckpoint(filepath='best_model_relevancy.h5', monitor='val_loss', save_best_only=True)]

model.compile(loss='categorical_crossentropy', optimizer="adam", metrics=['accuracy'])

history = model.fit(X_train, Y_train, epochs=100, batch_size=32, callbacks = callbacks, validation_data=(X_valid,Y_valid))
...
plt.title('Relevancy Classifier Accuracy per Epoch')
plt.ylabel('Accuracy')
plt.xlabel('Epoch')
...
plt.title('Relevancy Classifier Categorical Cross-entropy Loss per Epoch')
plt.ylabel('Loss')
plt.xlabel('Epoch')
plt.legend(['Train', 'Test'], loc='upper left')
plt.show()
...
plot_model(model, to_file='relevancy_model.png', show_shapes=True)
